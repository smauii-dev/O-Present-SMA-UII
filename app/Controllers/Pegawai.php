...
class Pegawai extends BaseController
{
    ...
    public function importExcel()
    {
        ...
        if (!$file->isValid() || !in_array($file->getExtension(), ['xls', 'xlsx'])) {
            session()->setFlashdata('gagal', 'Format file tidak valid. Gunakan file .xls atau .xlsx');
        ...
        // Ambil NIP terakhir sebelum looping dimulai
        $nip_terakhir = $this->pegawaiModel->getNIPPegawai();
        if (!empty($nip_terakhir)) {
            $nip_terakhir = explode('-', $nip_terakhir);
            $nomor_urut = (int)$nip_terakhir[1];
        } else {
            $nomor_urut = 0;
        }
        ...
        // Mulai looping data Excel dari baris ke-2
        foreach ($sheetData as $index => $row) {
            // Lewati baris 1 karena itu adalah header kolom
            if ($index == 1) continue; 

            // Jika baris (Kolom A) kosong, hentikan proses (mencegah insert data kosong)
            if (empty($row['A'])) break; 

            // 1. Increment Nomor NIP
            $nomor_urut++;
            $nip_baru = 'PEG-' . str_pad($nomor_urut, 4, 0, STR_PAD_LEFT);

            // 2. Ambil password mentah dari Excel (Kolom J) dan langsung di-hash
            $password_mentah = (string) $row['J'];
            $password_hash = $this->usersModel->hashPassword($password_mentah);
            
            // 3. Set akun otomatis aktif
            $active = 1;
            $activate_hash = null;
            ...
            $this->pegawaiModel->save([
                ...
                'foto' => 'default.jpg', // Sesuaikan jika field foto default kamu berbeda
            ]);
            ...
            // Simpan ke tabel role (auth_groups_users)
            $this->usersRoleModel->save([
                'group_id' => $row['I'],
                'user_id' => $user_id,
            ]);
        ...
        if ($db->transStatus() === false) {
            session()->setFlashdata('gagal', 'Terjadi kesalahan sistem. Data gagal diimport.');
        } else {
            session()->setFlashdata('berhasil', 'Data siswa berhasil diimport secara massal.');
        }
    ...
    public function downloadTemplate()
    {
        ...
        $worksheet->setCellValue('A1', 'Nama Siswa');
        $worksheet->setCellValue('B1', 'Jenis Kelamin (Perempuan/Laki-laki)');
        $worksheet->setCellValue('C1', 'Alamat');
        $worksheet->setCellValue('D1', 'No Handphone');
        $worksheet->setCellValue('E1', 'ID Jabatan (3 untuk siswa)');
        $worksheet->setCellValue('F1', 'ID Lokasi Presensi');
        $worksheet->setCellValue('G1', 'Email');
        $worksheet->setCellValue('H1', 'Username');
        $worksheet->setCellValue('I1', 'ID Role (3 untuk siswa)');
        
        // --- UBAH BAGIAN INI ---
        $worksheet->setCellValue('J1', 'Password'); 

        // Memberikan style Bold pada Header dan melebarkan kolom otomatis
        ...
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment;filename="Template_Bulk_Insert_Pegawai.xlsx"');
        header('Cache-Control: max-age=0');
